...
if test -f ${DESTDIR}/usr/local/apache_afl_cve//bin/apr-1-config; then
   APR_CONFIG=${DESTDIR}/usr/local/apache_afl_cve//bin/apr-1-config
   APU_CONFIG=${DESTDIR}/usr/local/apache_afl_cve//bin/apu-1-config
else
...
test -n "/usr/local/apache_afl_cve//bin/apu-1-config" && APU_INCLUDEDIR="`${APU_CONFIG} --includedir`"

installbuilddir="/usr/local/apache_afl_cve//build"
